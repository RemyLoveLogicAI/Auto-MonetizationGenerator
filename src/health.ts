...
/**
 * An asynchronous health check endpoint for the application.
 * Returns a JSON object with status, version, performance, and environment checks.
...
export async function health(c: Context) {
  const checks = {
    ...
    r2_bound: Boolean((c as any).env?.R2),
    env: 'pages',
  }
